...
function buildRules(settings) {
  return `
    ...
    <p>Each round, every student secretly picks a speed for their station.</p>
    <p>The whole shop runs at the slowest chosen speed, not the average speed.</p>
    <p>Everyone first receives the same team bonus based on the shop speed, then pays their own tiredness cost based on the speed they personally chose.</p>
  `;
}
    ...
    try {
      ...
      if (data.currentRoundSummary?.status === "closed") {
        ...
        roundResultText.innerHTML = `
          <strong>第 ${data.currentRoundSummary.number} 轮已结算</strong><br />
          ...
          平均选择速度是 <strong>${formatNumber(data.currentRoundSummary.averageChoice)}</strong> 档。${
            defaultCount
              ? `<br />有 ${defaultCount} 个工位未提交，系统按 1 档处理。`
              : ""
          }
        `;
      } else if (data.status === "collecting") {
        roundResultText.innerHTML = `
          <strong>第 ${data.currentRound} 轮进行中</strong><br />
          学生正在悄悄选择本轮速度。你可以等大家都提交后再结束本轮。
        `;
      } else {
        roundResultText.innerHTML = "还没有结算轮次。";
      }
        ...
        .map(
          (player) => `
            <div class="seat-tile">
              ...
              <div class="tiny">累计到手: ${formatNumber(player.cumulative)}</div>
            </div>
          `
        )
        .join("");
      ...
      if (!round) {
        roundDetailTable.innerHTML =
          `<tr><td colspan="6">还没有进行中的轮次或结果。</td></tr>`;
          ...
          .map(
            (player) => `
              <tr>
                ...
                <td>${submittedSeats.has(player.seat) ? "已提交" : "等待中"}</td>
                <td>--</td>
                <td>--</td>
                <td>--</td>
              </tr>
    ...
    speedChoices.innerHTML = Array.from({ length: maxSpeed }, (_, index) => {
      ...
      return `
        <button class="speed-choice" type="button" data-speed="${speed}">
          <strong>${speed} 档</strong>
          <span>${speed === 1 ? "最稳最慢" : speed === maxSpeed ? "最快最累" : "中间速度"}</span>
        </button>
      `;
    }).join("");
  ...
  async function refreshStudent() {
    try {
      ...
      if (!token) {
        ...
        setText("studentStationTitle", "你的工位：--");
        ...
        joinTip.textContent = joinOpen
          ? "请输入名字或代号后进入。"
          : "老师还没完成设置，或者第一轮已经开始。学生只能在开局前加入。";
        return;
      }

      const data = await request(`/api/milk-tea/student/state?token=${encodeURIComponent(token)}`);
      ...
      setText(
        "studentStatus",
        {
          setup: "等待设置",
          lobby: "等待老师开局",
          collecting: data.currentRoundSummary?.submitted ? "已提交" : "正在选择",
          results: "本轮已结算",
          finished: "全部结束",
        }[data.status] || data.status
      );
      setText("studentSeatBadge", `Seat ${data.player.seat} · ${data.player.station}`);
      setText("studentStationTitle", `你的工位：${data.player.station}`);
      ...
      if (data.status === "setup" || data.status === "lobby") {
        instruction.textContent = "老师还没有开始本轮。";
      } else if (data.status === "collecting") {
        instruction.textContent = data.currentRoundSummary?.submitted
          ? `第 ${data.currentRound} 轮你已经提交了 ${data.currentRoundSummary.ownSpeed} 档，请等待老师结算。`
          : `第 ${data.currentRound} 轮正在进行。请为你的工位选择 1 到 ${data.settings.maxSpeed} 档速度。`;
      } else if (data.status === "results") {
        instruction.textContent = data.currentRoundSummary?.defaulted
          ? "你本轮未提交，系统按 1 档处理。请查看这轮结算结果。"
          : "本轮已经结算。请查看团队奖金、辛苦值和你最后到手多少。";
      } else if (data.status === "finished") {
        instruction.textContent = "全部轮次已结束。你可以回看自己的全部记录。";
      }
